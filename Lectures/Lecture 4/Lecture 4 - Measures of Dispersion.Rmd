...
* Posting Professionalism Grades weekly
* Posting a list of formulas - will be allowed for tests and quizzes
* Problem Set 2 
        + Should take less time
        + Will be posted this evening
        + Due Wednesday
        + Will include: a couple of R questions, 1 or 2 problems using the same data step by step from sum to standard deviation, a couple of fill in the blanks about concepts
        + Not required to use R if a calculator works

* Problem Set 3 
        + Will be up by Monday; due next Friday
        + More from this week plus
        + Will include content through Wednesday
        + Percentiles, quartiles
        + basic probability/sets
        + basic frequency distribution concepts
...
#### Our two variables are arrests for assaulst and the percent of urban population. What makes sense as the *explanatory variable* or *independent variable* (x) and which makes sense as the *dependent variable* (y)?
...
cat('mean_assault_arrests is: ')
mean_assault_arrests
cat('mean_urban_population is: ')
...
Measures of dispersion typically look at how the data is scattered around the mean. Let's look at that visually. First the mean of *Urban Pop*:

```{r}

plot(arrests_data$UrbanPop,arrests_data$Assault, ylim = c(0,350))
abline(v = mean_urban_population)


```

Now, let's look at just the *Assault Arrest* and see how it's dispersed.
